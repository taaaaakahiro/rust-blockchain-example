...
impl ServerCore {
    ...
    pub fn join_network(&mut self) {
        ...
        connection_manager::join_network(
            &self.my_ip,
            &self.my_port,
            &self.core_node_ip,
            &self.core_node_port,
    ...
    fn handle_message(&mut self, msg: &str, locked: &mut bool, cur_time: &mut i64, w: &Worker) {
        ...
        println!(
            "received msg cmd ip port payload {} {} {} {}",
            cmd, ip, port, payload
        );
        ...
        if cmd == connection_manager::message_manager::MSG_REQUEST_FULL_CHAIN {
            ...
            let new_msg = connection_manager::get_message_text(
                connection_manager::message_manager::RSP_FULL_CHAIN,
                &self.my_ip,
                &self.my_port,
                &bc_str,
            );
            ...
            if self.connection_manager.is_core(&ip, &port) == false {
                ...
                self.tp.print();
                let new_msg = connection_manager::get_message_text(
                    connection_manager::message_manager::MSG_NEW_TRANSACTION,
                    &self.my_ip,
                    &self.my_port,
                    payload,
                );
            ...
            if new_block.is_valid_block(&self.previous_block_hash) == true {
                ...
                let msg = connection_manager::get_message_text(
                    connection_manager::message_manager::MSG_REQUEST_FULL_CHAIN,
                    &self.my_ip,
                    &self.my_port,
                    "",
                );
            ...
            if new_block.is_valid_block(&self.previous_block_hash) == true {
                ...
                self.bc.set_new_block(new_block);
                let msg = connection_manager::get_message_text(
                    connection_manager::message_manager::MSG_NEW_BLOCK,
                    &ip,
                    &port,
                    &payload,
                );
                ...
                if flag == true {
                    ...
                    if orphan_blocks.chain.len() != 0 {
                        let new_transactions =
                            self.bc.get_transactions_from_orphan_blocks(&orphan_blocks);
    ...
    pub fn generate_block_with_tp(
        ...
        if flag == true {
            ...
            let msg = connection_manager::get_message_text(
                connection_manager::message_manager::MSG_NEW_BLOCK_TO_ALL,
                &my_ip,
                &my_port,
                &new_block_string,
            );
        ...
        let msg2 = connection_manager::get_message_text(
            connection_manager::message_manager::MSG_UNLOCKED,
            &my_ip,
            &my_port,
            &cur_time.to_string(),
        );
            ...
            if t.tx_type == false {
                ...
                if counter != 0 {
                    ...
                    if insentive != fee_for_block {
                        println!(
                            "Invalid value in fee for Coinbase Transaction {}",
                            insentive
                        );
                        return false;
                    }
    ...
    fn wait_for_access(&self) {
        ...
        thread::spawn(move || loop {
            ...
            if locked == false && Local::now().timestamp() - cur_time > MINING_INTERVAL {
                ...
                thread::spawn(move || {
                    ServerCore::generate_block_with_tp(
                        transaction_pool_clone,
                        km_clone,
                        previous_block_hash_clone,
                        my_ip_tmp,
                        my_port_tmp,
                        tx_clone,
                        &to_stop,
            ...
            if iter == CHECK_PEERS_CONNECTION_INTERVAL {
                server_core
                    .connection_manager
                    .check_peers_connection(tx.clone());
                iter = 0;
            }
            iter += 1;
...
pub fn verify_sbc_transaction_sig(
    transaction: &Transaction,
) -> (bool, Vec<block_chain::TransactionOutput>) {
...
fn get_pubkey_from_sbc_transaction(
    transaction: &Transaction,
) -> (String, Vec<block_chain::TransactionOutput>) {
...
impl ClientCore {
    ...
    pub fn send_req_full_chain_to_my_core_node(&mut self) {
        ...
        let new_message = connection_manager::get_message_text(
            connection_manager::message_manager::MSG_REQUEST_FULL_CHAIN,
            &self.my_ip,
            &self.my_port,
            "",
    ...
    pub fn handle_message(&mut self, msg: &str, tx: mpsc::Sender<String>) {
        ...
        if cmd == message_manager::RSP_FULL_CHAIN {
            ...
            let (new_block_chain, ortphan_blocks, flag) =
                self.bc.resolve_conflicts(new_block_chain);
    ...
    pub fn wait_for_access_4edge(&mut self, tx_main: mpsc::Sender<String>) {
        ...
        let mut client_core = client_server_core::ClientCore::create(
            &self.my_ip,
            &self.my_port,
            &self.core_node_ip,
            &self.core_node_port,
